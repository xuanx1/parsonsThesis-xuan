// SEA country boundaries are loaded from data/sea_countries.geojson at runtime
// (see the L.geoJSON block below). The previous import { combinedCoordinates }
// from "./sea_coord.js" was removed in cleanup.

// =============================================================================
// A* feasibility router (replaces the curve+jitter placeholder).
// Loads data/preCal/grid_final.geojson once, then routes between any two
// lng/lat pairs over the pre-computed feasibility grid. Slider positions
// (tsi-filter, sdi-filter, e2i-filter, opi-filter, pei-filter) act as cost
// weights: higher slider = stronger preference for cells with high score on
// that index. Drag a slider, recalculate, the path bends accordingly.
// Cell adjacency uses spatial bucketing rather than a regular lattice, so an
// irregular/clipped grid still routes correctly.
// =============================================================================
// Stage 4A: Leaflet polyline + traveling dot that mirror the shim's "ffi-path"
// source. The shim still stores the source data so existing readers (report,
// elevation profile, coordinates table) keep working; these objects are what
...
async function __ensureFeasibility() {
  ...
  __FEAS.ready = (async () => {
    ...
    // 2.5× step lets each cell reach up to its second ring of neighbours,
    // which bridges the small water gaps between disconnected components
    // (Singapore↔Johor, Bali↔Java, Sumatra↔peninsula etc) that would
    // otherwise leave island cells unreachable from a mainland start.
...
async function __aStarRoute(startLngLat, endLngLat) {
  ...
  console.log("[A*] start cell:", sIdx, "@", cells[sIdx]?.cx, cells[sIdx]?.cy, "neighbors:", cells[sIdx]?.neighbors?.length);
  console.log("[A*] goal cell:", gIdx, "@", cells[gIdx]?.cx, cells[gIdx]?.cy, "neighbors:", cells[gIdx]?.neighbors?.length);
  ...
  while (open.size() > 0) {
    ...
    if (idx === gIdx) {
      ...
      console.log(`A* route: ${path.length} cells, expanded ${expanded.toLocaleString()} in ${ms} ms`);
      return { path, coords: path.map(i => [cells[i].cx, cells[i].cy]) };
    }
    if (expanded > maxExpanded) { console.warn("[A*] hit expansion cap", maxExpanded); break; }
    const cur = cells[idx];
  ...
  console.warn("[A*] open set exhausted; visited", closed.size, "of", cells.length, "cells — start and goal are in disconnected components");
...
// =============================================================================
// Stage 1: Mapbox -> Leaflet basemap swap.
// Map is now a Leaflet L.Map; CARTO dark tiles via OpenStreetMap (no API key).
// A compatibility shim further down in this block stubs the Mapbox-specific
// methods (addSource/addLayer/setLayoutProperty/Marker/Popup/etc.) so the
// existing layer-toggle code keeps running while Stages 2-4 port each layer.
// 3D pitch/bearing/terrain are gone (Leaflet is 2D); everything else stays.
// =============================================================================
const map = L.map("map", {
  center: [7.0, 113.0],                 // [lat, lng] — centred on SEA
  zoom: 5,                              // closer initial framing
  minZoom: 4,                           // can't zoom out past SEA
  maxZoom: 14,
  maxBounds: [[-15, 85], [30, 145]],    // tight pan-bound around SEA
  maxBoundsViscosity: 1.0,              // bounce back hard at the edge
  zoomControl: false,                   // custom +/- buttons below stay
  attributionControl: true,
  preferCanvas: true,                   // helps when many features render
});

// OpenTopoMap (free, hillshaded relief, no API key, OSM-derived).
// CC-BY-SA — attribution required.
L.tileLayer("https://{s}.tile.opentopomap.org/{z}/{x}/{y}.png", {
  subdomains: "abc",
  attribution: 'Map © <a href="https://opentopomap.org">OpenTopoMap</a> (CC-BY-SA) · Data © <a href="https://www.openstreetmap.org/copyright">OSM</a>',
  maxZoom: 17,
}).addTo(map);

// Custom pane for the calculated route — sits above markerPane (600) so the
// drawn route line is visible on top of every overlay and marker.
map.createPane("routePane");
map.getPane("routePane").style.zIndex = 650;
map.getPane("routePane").style.pointerEvents = "auto";

// -----------------------------------------------------------------------------
// Mapbox compatibility shim. Each Stage (2-4) deletes the corresponding
// shimmed call sites and replaces them with real Leaflet code; the shim
// shrinks accordingly and disappears at the end of Stage 4.
// -----------------------------------------------------------------------------
window.mapboxgl = window.mapboxgl || {};
window.mapboxgl.accessToken = "";

// __MBX_SOURCES is the canonical store for ffi-path data — populateCoordinatesTable,
// generateElevationProfile and the elevation-inset read route coords from here.
const __MBX_SOURCES = new Map();
map.addSource = (id, src) => { __MBX_SOURCES.set(id, src); };
map.removeSource = (id) => { __MBX_SOURCES.delete(id); };
map.getSource = (id) => {
  const src = __MBX_SOURCES.get(id);
  if (!src) return null;
  return {
    _data: src.data,
    setData: (data) => { src.data = data; },
  };
};

// Polymorphic addLayer / removeLayer: real Leaflet Layer instances delegate to
// Leaflet's natives (so layer.addTo(map) keeps working), Mapbox-style configs
// are no-op'd. Without this discrimination, our shim would shadow Leaflet's
// natives and every subsequent `.addTo(map)` would silently do nothing.
const __leafletAddLayer = map.addLayer.bind(map);
const __leafletRemoveLayer = map.removeLayer.bind(map);
map.addLayer = (cfgOrLayer) => {
  if (cfgOrLayer instanceof L.Layer) return __leafletAddLayer(cfgOrLayer);
};
map.removeLayer = (idOrLayer) => {
  if (idOrLayer instanceof L.Layer) return __leafletRemoveLayer(idOrLayer);
};

// Stage 2: real Leaflet layer registry + visibility wiring.
// Each ported layer registers via __registerLeafletLayer(id, leafletLayer, defaultVisible).
// The existing toggle buttons call map.setLayoutProperty(id,"visibility",v) — that
// now genuinely adds/removes the Leaflet layer.
const __LFLT_LAYERS = new Map(); // id -> { layer, loader, visible, desired, defaultVisible }
// layerOrLoader: either an L.Layer (eager) OR a function that returns a Promise<L.Layer> (lazy).
// Lazy layers don't fetch until the user first toggles them visible.
function __registerLeafletLayer(id, layerOrLoader, defaultVisible = true) {
  if (typeof layerOrLoader === "function") {
    __LFLT_LAYERS.set(id, {
      layer: null,
      loader: layerOrLoader,
      loadPromise: null,
      visible: false,
  ...
  __LFLT_LAYERS.set(id, {
    ...
    defaultVisible,
    desired: defaultVisible ? "visible" : "none",
  });
  if (defaultVisible) layerOrLoader.addTo(map);
  return layerOrLoader;
}

map.getLayer = (id) => __LFLT_LAYERS.get(id);
map.setLayoutProperty = (id, prop, value) => {
  if (prop !== "visibility") return;
  const e = __LFLT_LAYERS.get(id);
  if (!e) return;
  e.desired = value;
  if (value === "visible") {
    if (e.layer) {
      if (!e.visible) { e.layer.addTo(map); e.visible = true; }
    } else if (e.loader && !e.loadPromise) {
      // First show on a lazy layer: kick off the fetch. Race-safe: if user
      // hides before the fetch resolves, e.desired is "none" and we skip add.
      e.loadPromise = Promise.resolve(e.loader())
        .then((layer) => {
          e.layer = layer;
          if (e.desired === "visible" && layer) { layer.addTo(map); e.visible = true; }
        })
        .catch((err) => console.error(`lazy layer "${id}" failed:`, err));
    }
  } else if (value === "none") {
    if (e.visible && e.layer) { map.removeLayer(e.layer); e.visible = false; }
  }
};
map.getLayoutProperty = (id, prop) => {
  if (prop !== "visibility") return undefined;
  const e = __LFLT_LAYERS.get(id);
  return e ? (e.visible ? "visible" : "none") : undefined;
};
...
map.getStyle = () => ({ layers: [] });
map.addControl = () => {};
map.getCanvas = () => map.getContainer();
map.resize = () => map.invalidateSize();
map.loaded = () => true;

// Translate Mapbox-style flyTo / fitBounds (lng/lat) to Leaflet (lat/lng).
const __leafletFlyTo = map.flyTo.bind(map);
const __leafletFitBounds = map.fitBounds.bind(map);
map.flyTo = (opts) => {
  if (!opts) return map;
  const c = opts.center;
  if (Array.isArray(c) && c.length >= 2) {
    __leafletFlyTo([c[1], c[0]], opts.zoom ?? map.getZoom(), { duration: (opts.duration || 1500) / 1000 });
  }
  return map;
};
map.fitBounds = (b, opts) => {
  ...
  if (Array.isArray(b) && b.length === 4 && typeof b[0] === "number") {
    // Mapbox/turf bbox: [west, south, east, north]
    bounds = [[b[1], b[0]], [b[3], b[2]]];
  }
  const fitOpts = { ...(opts || {}) };
  if (typeof fitOpts.padding === "number") {
    fitOpts.padding = [fitOpts.padding, fitOpts.padding];
  }
  __leafletFitBounds(bounds, fitOpts);
  return map;
};

// "load" / "style.load" / "sourcedata" -> fire on whenReady.
// IMPORTANT: forward the (fn, context) pair to Leaflet's native on/off/once.
// Leaflet internals call map.on('move', this._update, this) — dropping the
// context argument breaks any callback that relies on `this` (the Scale
// control's _update is the loud one — every pan throws TypeError).
const __leafletOn = map.on.bind(map);
map.on = (ev, a, b, c) => {
  if (ev === "load" || ev === "style.load" || ev === "idle") {
    if (typeof a === "function") map.whenReady(a, b);
    else if (typeof b === "function") map.whenReady(b, c);
    return map;
  }
  if (ev === "sourcedata" || ev === "data") {
    // Mapbox fires sourcedata when a specific source updates. Leaflet has no
    // equivalent — and translating to 'layeradd' cascades disastrously
    // (every Stage 2-4 layer add fires every sourcedata handler, including
    // the one that calls elevationMap.flyTo on the hidden inset map, which
    // then throws NaN errors that abort layer rendering). Stage 4A already
    // calls populateCoordinatesTable + elevation-map updates directly in
    // drawRailLines, so the handlers are redundant. No-op.
    return map;
  }
  if (typeof a === "string") {
    // Mapbox layer-scoped events ('mouseenter', 'click', etc., layerId, fn).
    // Leaflet has no equivalent on the map; ignore.
    return map;
  }
  __leafletOn(ev, a, b);
  return map;
};
const __leafletOff = map.off.bind(map);
map.off = (ev, a, b) => {
  if (typeof a === "string") return map; // layer-scoped
  __leafletOff(ev, a, b);
  return map;
};
const __leafletOnce = map.once.bind(map);
map.once = (ev, fn, ctx) => {
  if (ev === "load" || ev === "style.load" || ev === "idle") {
    map.whenReady(fn, ctx);
    return map;
  }
  __leafletOnce(ev, fn, ctx);
  return map;
};

// Mapbox click events expose event.lngLat.{lng,lat}. Leaflet exposes
// event.latlng.{lng,lat}. Bridge the property so existing handlers work.
__leafletOn("click", (e) => { e.lngLat = e.latlng; });
__leafletOn("mousemove", (e) => { e.lngLat = e.latlng; });

// mapboxgl.Marker -> L.marker shim. Existing call sites get ported one by
// one; this stub keeps any leftover invocations from crashing.
window.mapboxgl.Marker = function MapboxglMarkerShim(opts) {
  const el = opts && opts.element ? opts.element : null;
  let marker = null;
  this.setLngLat = function (ll) {
    const lat = Array.isArray(ll) ? ll[1] : ll.lat;
    const lng = Array.isArray(ll) ? ll[0] : ll.lng;
    const icon = el
      ? L.divIcon({ html: el.outerHTML, className: "", iconSize: [el.width || 40, el.height || 40], iconAnchor: [(el.width || 40) / 2, el.height || 40] })
      : new L.Icon.Default();
    marker = L.marker([lat, lng], { icon });
    return this;
  };
  this.addTo = function (m) { if (marker) marker.addTo(m); return this; };
  this.remove = function () { if (marker) marker.remove(); return this; };
  this.setPopup = function (popup) { if (marker && popup && popup._content) marker.bindPopup(popup._content); return this; };
  return this;
};

// mapboxgl.Popup -> L.popup shim.
window.mapboxgl.Popup = function MapboxglPopupShim(opts) {
  const popup = L.popup({ closeButton: opts?.closeButton !== false, autoClose: opts?.closeOnClick !== false });
  this._content = "";
  this.setLngLat = function (ll) {
    const lat = Array.isArray(ll) ? ll[1] : ll.lat;
    const lng = Array.isArray(ll) ? ll[0] : ll.lng;
  ...
  this.setHTML = function (html) { this._content = html; popup.setContent(html); return this; };
  this.setText = function (txt) { this._content = txt; popup.setContent(txt); return this; };
  this.addTo = function (m) { popup.openOn(m); return this; };
  this.remove = function () { popup.close(); return this; };
  this.getElement = function () { return popup.getElement() || document.createElement("div"); };
  return this;
};

window.mapboxgl.NavigationControl = function () { this.onAdd = () => document.createElement("div"); };
window.mapboxgl.AttributionControl = function () { this.onAdd = () => document.createElement("div"); };
window.mapboxgl.ScaleControl = function () { this.onAdd = () => document.createElement("div"); };
// Inert Map constructor used by the elevation inset (line ~7000); Stage 4 ports.
window.mapboxgl.Map = function MapboxglMapShim() {
  const noop = () => this;
  return new Proxy(this, {
    get(_t, prop) {
      if (prop === "loaded") return () => true;
      if (prop === "getCenter") return () => ({ lng: 0, lat: 0 });
      if (prop === "getZoom") return () => 1;
      return noop;
    },
  });
};
// =============================================================================
// (end Stage 1 basemap + shim)
// =============================================================================
...
// Compass dropped (no bearing in Leaflet 2D). Custom +/- buttons above stay.
// Scale: Leaflet's L.control.scale shows km + mi natively.
L.control.scale({ metric: true, imperial: true, position: "bottomleft", maxWidth: 200 }).addTo(map);

// SEA outline + black-out cover: load country polygons from a real GeoJSON
// file and render the white outline + a screen-spanning black overlay with
// each country's outer ring punched out (Leaflet polygons accept holes).
fetch("data/sea_countries.geojson")
  .then((r) => r.json())
  .then((fc) => {
    L.geoJSON(fc, {
      style: { color: "#ffffff", weight: 0.5, fill: false },
      interactive: false,
    }).addTo(map);
  ...
  .catch((err) => console.error("Failed to load sea_countries.geojson:", err));

// Stage 1: 3D terrain DEM + fog removed (Leaflet is 2D).
// Hillshading can be added later as a tile overlay (e.g. ESRI World Hillshade)
// if you want depth back.
...
map.on("click", (event) => {
  if (clickEnabled) {
    ...
    // add new origin marker (Leaflet)
    const originIcon = L.icon({
      iconUrl: "images/marker_o.svg",
      iconSize: [40, 40],
      iconAnchor: [20, 40],
    });
    originMarker = L.marker([coordinates.lat, coordinates.lng], { icon: originIcon }).addTo(map);
...
map.on("click", (event) => {
  if (destinationClickEnabled) {
    ...
    // add new destination marker (Leaflet)
    const destIcon = L.icon({
      iconUrl: "images/marker_d.svg",
      iconSize: [40, 40],
      iconAnchor: [20, 40],
    });
    destinationMarker = L.marker([coordinates.lat, coordinates.lng], { icon: destIcon }).addTo(map);
...
function fetchLocations(query, callback) {
  // Forward geocoding via Nominatim, bounded to SEA. Free, no API key.
  // viewbox order: left,top,right,bottom (lng,lat,lng,lat)
  ...
  fetch(url, { headers: { "Accept-Language": "en" } })
    ...
    .then((data) => {
      const locations = (data || []).map((r) => ({
        name: r.display_name,
        coordinates: [parseFloat(r.lon), parseFloat(r.lat)], // keep [lng,lat] for downstream code
      }));
      callback(locations);
    })
        ...
        if (locations.length > 0) {
          ...
          locations.forEach((location) => {
            ...
            suggestionItem.addEventListener("click", () => {
              ...
              // Drop a pin on the map (Leaflet)
              const pinIcon = L.icon({
                iconUrl: pinColor === "blue" ? "images/marker_o.svg" : "images/marker_d.svg",
                iconSize: [40, 40],
                iconAnchor: [20, 40],
              });
              const lat = location.coordinates[1];
              const lng = location.coordinates[0];
              markerReference.marker = L.marker([lat, lng], { icon: pinIcon }).addTo(map);
...
// Roads layer dropped (Stage 4G): used Mapbox vector tiles which Leaflet
// doesn't render natively. CARTO basemap already shows roads at the higher
// zoom levels.

// coastlines (Stage 2: real Leaflet layer)
fetch("data/earth-coastlines.geo.json")
  .then((r) => r.json())
  .then((data) => {
    const layer = L.geoJSON(data, {
      renderer: L.canvas(),
      style: { color: "#00ffff", weight: 1, opacity: 0.8, fill: false },
      interactive: false,
    });
    __registerLeafletLayer("sea-coastline-layer", layer, true);
  })
...
d3.json("data/worldQuakesMiles.json").then((data) => {
  ...
  // Stage 2: real Leaflet layer, default off (matches the original which set
  // visibility to "none" right after creation).
  const earthquakeLayer = L.geoJSON(geoData, {
    renderer: L.canvas(),
    pointToLayer: (f, latlng) => L.circleMarker(latlng, {
      radius: Math.max(2, Math.min(8, (f.properties.mag || 4) - 2)),
      color: "#ff7900",
      weight: 0,
      fillColor: "#ff7900",
      fillOpacity: 0.3,
  ...
  __registerLeafletLayer("earthquake-points", earthquakeLayer, false);
  ...
  // Initial visibility "none" already handled by __registerLeafletLayer above.
});
...
d3.tsv("data/tsunami.tsv").then((data) => {
  ...
  // Stage 2: real Leaflet tsunami layer, off by default (matches original).
  const tsunamiLayer = L.geoJSON(geoData, {
    renderer: L.canvas(),
    pointToLayer: (f, latlng) => L.circleMarker(latlng, {
      radius: Math.max(4, Math.min(20, (f.properties.mag || 0) * 2 + 4)),
      color: "#6dbefe",
      weight: 1,
      fillColor: "#6dbefe",
      fillOpacity: 0.4,
  ...
  __registerLeafletLayer("tsunami-points", tsunamiLayer, false);
...
// Stage 2: SEA regional border (fill + outline), always visible.
fetch("data/sea.json")
  .then((response) => response.json())
  .then((data) => {
    const fillLayer = L.geoJSON(data, {
      renderer: L.canvas(),
      style: { color: "transparent", weight: 0, fillColor: "#000000", fillOpacity: 0.3 },
      interactive: false,
    });
    const outlineLayer = L.geoJSON(data, {
      renderer: L.canvas(),
      style: { color: "#ffffff", weight: 1, opacity: 0.7, fill: false },
      interactive: false,
    });
    __registerLeafletLayer("sea-layer", fillLayer, true);
    __registerLeafletLayer("sea-outline-layer", outlineLayer, true);
  })
  .catch((error) => console.error("Error loading sea.json:", error));

// Stage 4B: Spatial population — replaces 12 private Mapbox tilesets with a
// single Leaflet polygon overlay on grid_final.geojson, shaded by gdp.
// Filter cells to gdp >= 50 (lower cells would render transparent anyway);
// drops feature count from ~49K to ~10K so first render is fast.
let __populationLayerPromise = null;
function __getPopulationLayer() {
  if (__populationLayerPromise) return __populationLayerPromise;
  __populationLayerPromise = fetch("data/preCal/grid_final.geojson")
    .then((r) => r.json())
    .then((fc) => {
      const filtered = { type: "FeatureCollection", features: fc.features.filter((f) => (f.properties.gdp || 0) >= 50) };
      console.log(`population layer: rendering ${filtered.features.length} of ${fc.features.length} cells (gdp >= 50)`);
      return L.geoJSON(filtered, {
        renderer: L.canvas(),
        style: (f) => {
          const g = f.properties.gdp || 0;
          let color = "#fef0d9", op = 0.35;
          if (g >= 5000) { color = "#7f0000"; op = 0.7; }
          else if (g >= 1500) { color = "#d7301f"; op = 0.6; }
          else if (g >= 500)  { color = "#fc8d59"; op = 0.55; }
          else if (g >= 150)  { color = "#fdcc8a"; op = 0.45; }
          return { color: "transparent", weight: 0, fillColor: color, fillOpacity: op };
        },
        interactive: false,
      });
    });
  return __populationLayerPromise;
}

map.on("load", function () {
  // Register ONE entry (raster-layer-0). The original 12-iteration toggle
  // is collapsed to a single setLayoutProperty call below — saves 11
  // unnecessary add/remove cycles on a heavy canvas layer per click.
  __registerLeafletLayer("raster-layer-0", () => __getPopulationLayer(), false);
  Array.from({ length: 1 }).forEach((_, index) => {
    const layerId = `raster-layer-0`;
    // (registration done above; loop kept only to preserve the surrounding
    // closure structure so the trailing `});` braces still match)
    void layerId; void index;
    ...
    const toggleButton = toggleContainer
      ...
      .on("click", () => {
        ...
        if (visibility === "visible") {
          ...
          toggleButton.style("filter", "brightness(30%)");
        } else {
...
map.on("style.load", () => {
  ...
  // Stage 3: lazy-loaded (6 MB) — only fetches on first toggle click.
  __registerLeafletLayer("avgHU-layer", () => {
    return fetch("data/avgHU_vect.geojson")
      .then((r) => r.json())
      .then((data) => L.geoJSON(data, {
        ...
        style: (f) => {
          ...
          let color = "#ffffff";
          if (dn >= 210) color = "#800080";
          else if (dn >= 198) color = "#ff0000";
          else if (dn >= 183) color = "#ffa500";
          else if (dn >= 160) color = "#ffff00";
          return { color: "transparent", weight: 0, fillColor: color, fillOpacity: 0.5 };
        },
        interactive: false,
      }));
  }, false);

  // The original then-block continues to build the toggle, legend and hover
  // description; rewrap with a resolved promise so the structure stays.
  Promise.resolve()
    .then(() => {
...
// Stage 3: lazy-loaded (79 MB!) — only fetches on first toggle click.
map.on("load", function () {
  __registerLeafletLayer("forest-layer", () => {
    return fetch("data/sea_forest_vect.geojson")
      .then((r) => r.json())
      .then((data) => L.geoJSON(data, {
        renderer: L.canvas(),
        style: (f) => {
          const dn = f.properties.DN || 0;
          let color = "#e5f5e0";
          if (dn >= 105) color = "#00441b";
          else if (dn >= 48) color = "#006d2c";
          else if (dn >= 30) color = "#41ab5d";
          else if (dn >= 12) color = "#a1d99b";
          return { color: "transparent", weight: 0, fillColor: color, fillOpacity: 0.3 };
        },
        interactive: false,
      }));
  }, false);
...
map.on("style.load", () => {
  ...
  fetch("data/amphibians_vect.geojson")
    .then((response) => response.json())
    .then((data) => {
      // Stage 3: real Leaflet layer, default off.
      const amphibiansLayer = L.geoJSON(data, {
        renderer: L.canvas(),
        style: (f) => {
          const dn = f.properties.DN || 0;
          let color = "#ffffcc";
          if (dn >= 5) color = "#fd8d3c";
          else if (dn >= 3) color = "#feb24c";
          else if (dn >= 2) color = "#fed976";
          else if (dn >= 1) color = "#ffeda0";
          return { color: "transparent", weight: 0, fillColor: color, fillOpacity: 0.5 };
        },
        interactive: false,
      });
      __registerLeafletLayer("amphibians-layer", amphibiansLayer, false);
...
map.on("style.load", () => {
  ...
  fetch("data/birds_vect.geojson")
    ...
    .then((data) => {
      // Stage 3: real Leaflet layer, default off.
      const birdsLayer = L.geoJSON(data, {
        renderer: L.canvas(),
        style: (f) => {
          const dn = f.properties.DN || 0;
          let color = "#ffffff";
          if (dn >= 230) color = "#cc3300";
          else if (dn >= 209) color = "#ff6600";
          else if (dn >= 186) color = "#ff9933";
          else if (dn >= 140) color = "#ffcc99";
          return { color: "transparent", weight: 0, fillColor: color, fillOpacity: 0.8 };
        },
        interactive: false,
      });
      __registerLeafletLayer("birds-layer", birdsLayer, false);
...
map.on("style.load", () => {
  ...
  fetch("data/mammals_vect1.geojson")
    ...
    .then((data) => {
      // Stage 3: real Leaflet layer, default off.
      const mammalsLayer = L.geoJSON(data, {
        renderer: L.canvas(),
        style: (f) => {
          const dn = f.properties.DN || 0;
          let color = "#d9b3ff";
          if (dn >= 17) color = "#800080";
          else if (dn >= 14) color = "#a64dff";
          else if (dn >= 10) color = "#bf80ff";
          else if (dn >= 0) color = "#d9b3ff";
          return { color: "transparent", weight: 0, fillColor: color, fillOpacity: 0.4 };
        },
        interactive: false,
      });
      __registerLeafletLayer("mammals-layer", mammalsLayer, false);
...
// Stage 4D: country-level airports / railways / sea-ports.
// Each HOTOSM GeoJSON becomes a Leaflet layer added eagerly, no toggle
// (matches original behaviour). Same colour scheme: yellow airports, blue
// seaports, red railways.
map.on("style.load", () => {
  CountryGeojson.forEach((file) => {
    fetch(file)
      ...
      .then((geojson) => {
        let layer;
        if (file.includes("railways")) {
          layer = L.geoJSON(geojson, {
            renderer: L.canvas(),
            style: { color: "#ff0000", weight: 3, opacity: 0.5 },
            interactive: false,
          });
        } else if (file.includes("airports")) {
          layer = L.geoJSON(geojson, {
            renderer: L.canvas(),
            pointToLayer: (f, ll) => L.circleMarker(ll, {
              radius: 4, color: "#ffff00", weight: 0, fillColor: "#ffff00", fillOpacity: 0.2,
            }),
            interactive: false,
          });
        } else if (file.includes("sea_ports")) {
          layer = L.geoJSON(geojson, {
            renderer: L.canvas(),
            pointToLayer: (f, ll) => L.circleMarker(ll, {
              radius: 4, color: "#0000ff", weight: 0, fillColor: "#0000ff", fillOpacity: 0.1,
            }),
            interactive: false,
          });
        }
...
// Stage 4C: Spatial GDP — replaces the private Mapbox raster tileset with
// a Leaflet polygon overlay on grid_final.geojson, shaded by gdp using a
// peach ramp (matching the original "#FFDFBF" tint).
map.on("load", function () {
  const layerId = "gdp-raster-layer";

  __registerLeafletLayer(layerId, () => {
    return fetch("data/preCal/grid_final.geojson")
      .then((r) => r.json())
      .then((fc) => {
        const filtered = { type: "FeatureCollection", features: fc.features.filter((f) => (f.properties.gdp || 0) >= 30) };
        console.log(`gdp layer: rendering ${filtered.features.length} of ${fc.features.length} cells (gdp >= 30)`);
        return L.geoJSON(filtered, {
          renderer: L.canvas(),
          style: (f) => {
            const g = f.properties.gdp || 0;
            let color = "#fff0e0", op = 0.3;
            if (g >= 3000) { color = "#a86329"; op = 0.65; }
            else if (g >= 1000) { color = "#d18a4f"; op = 0.55; }
            else if (g >= 300)  { color = "#e8b07b"; op = 0.45; }
            else if (g >= 100)  { color = "#FFDFBF"; op = 0.4; }
            return { color: "transparent", weight: 0, fillColor: color, fillOpacity: op };
          },
          interactive: false,
        });
...
// Stage 4E: major cities as Leaflet markers with name labels.
// Original was a Mapbox symbol layer filtered by pitch >= 60 + zoom <= 8.
// No pitch in Leaflet, so we honour the zoom condition only.
map.on("load", () => {
  // Inject a tiny CSS rule for the label halo (replicates Mapbox text-halo).
  const __cityCss = document.createElement("style");
  __cityCss.textContent = `
    .city-label{color:#fff;font-weight:700;font-size:13px;
      text-shadow:0 0 2px #000,0 0 2px #000,0 0 2px #000,0 0 2px #000;
      white-space:nowrap;pointer-events:none;}
    .city-label-icon{background:none;border:none;}`;
  document.head.appendChild(__cityCss);

  const __cityGroup = L.layerGroup();
  majorCities.features.forEach((f) => {
    const [lng, lat] = f.geometry.coordinates;
    const m = L.marker([lat, lng], {
      icon: L.divIcon({
        html: `<span class="city-label">${f.properties.name}</span>`,
    ...
    __cityGroup.addLayer(m);
  });

  const __updateCityVis = () => {
    if (map.getZoom() <= 8) {
      if (!map.hasLayer(__cityGroup)) __cityGroup.addTo(map);
    } else if (map.hasLayer(__cityGroup)) {
      map.removeLayer(__cityGroup);
    }
  };
  map.on("zoomend", __updateCityVis);
  __updateCityVis();
});
...
// Stage 4F: elevation lookup with a fallback chain across multiple free,
// no-API-key sources. Order: Open-Elevation → Open-Meteo → Open-Topo-Data.
// Chunked at 100 coords/request; aborts after 8 s per source so a stalled
// service doesn't block the next one. coords array is [lng, lat] pairs.
async function __fetchElevations(coords) {
  ...
  // 1) Open-Elevation: pipe-separated, no real chunk limit but be polite
  let elevs = await tryAllChunks(
    100,
  ...
  // 2) Open-Meteo: separate lat/lng arrays, max 100 per request
  elevs = await tryAllChunks(
    100,
    (chunk) => {
  ...
  // 3) Open-Topo-Data: SRTM30m, max 100 per request, ~1 req/sec
  elevs = await tryAllChunks(
    100,
...
// Single-point convenience wrapper (used by getElevationScore / getElevationScoreOPI).
// All call sites for these scores are commented out, but kept for completeness.
...
async function getPopulationDensityScore(coordinates) {
  ...
  for (const tilesetId of tilesets) {
    ...
    // Dead code: every call site to getPopulationDensityScore is commented
    // out. URL neutered so an accidental call can't hit Mapbox.
    const url = `data:,`;
...
resetButton.addEventListener("click", () => {
  ...
  // Stage 4A: also clear the Leaflet polyline + dot.
  if (__routePolyline) { __routePolyline.remove(); __routePolyline = null; }
  if (__routeDot) { __routeDot.remove(); __routeDot = null; }
  ...
  // reset map view to center of Southeast Asia (Leaflet [lat,lng])
  map.flyTo([5.0, 110.0], 4);
...
routeButton.addEventListener("click", async () => {
  ...
  if (originMatch && destinationMatch) {
    ...
    async function calculateCurvedPath(start, end) {
      console.log("[route] calculateCurvedPath start", { start, end });
      // Real router: A* over the pre-computed feasibility grid. The slider
      // positions act as cost weights, so dragging a slider up biases the path
      // toward cells that score high on that index.
      ...
      try {
        ...
        const out = [start, ...coords, end];
        console.log("[route] A* succeeded, returning", out.length, "coords");
        return out;
      } catch (err) {
        console.warn("[route] A* failed, falling back to curve:", err);
        __lastRouteCellPath = [];
        const path = [];
    ...
    const getPopulationCountFromRoute = async () => {
      // Sum gdp over the cells the A* route passes through. Multiplied by a
      // calibration factor so the displayed number reads as a population-served
      // estimate. Replaces the 12 private Mapbox tilesets.
      if (!__lastRouteCellPath || __lastRouteCellPath.length === 0) {
        console.log("No A* cell path available to estimate population.");
        return "0";
      }
      const POP_CALIBRATION = 100; // gdp unit ≈ 100 residents (rough)
      ...
      populationServedElement.textContent = formattedPopulation;
      populationServedElement.style.display = 'none';
      if (!document.getElementById('population-served')) document.body.appendChild(populationServedElement);
    ...
    function animatePath(sourceId, coordinates) {
      return new Promise(resolve => {
        ...
        // Stage 4A: real Leaflet traveling dot.
        const startLatLng = [coordinates[0][1], coordinates[0][0]];
        if (!__routeDot) {
          __routeDot = L.circleMarker(startLatLng, {
            radius: 6,
            color: '#00ff66',
            weight: 2,
            fillColor: '#ffffff',
          ...
          __routeDot.setLatLng(startLatLng);
          if (!map.hasLayer(__routeDot)) __routeDot.addTo(map);
        }

        // Shim still stores 'traveling-dot' source so any reset code that
        // calls map.getLayer('traveling-dot-layer') still finds something.
        if (!map.getSource('traveling-dot')) {
          map.addSource('traveling-dot', {
            type: 'geojson',
            data: { type: 'Feature', geometry: { type: 'Point', coordinates: coordinates[0] } },
          });
          map.addLayer({ id: 'traveling-dot-layer', type: 'circle', source: 'traveling-dot' });
        } else {
        ...
        function drawNextSegment() {
          if (currentPoint < totalPoints) {
            // Update the path data on the shim source (read by report/table).
            const animatedCoords = coordinates.slice(0, currentPoint + 1);
            map.getSource(sourceId).setData({
              type: "Feature",
              geometry: { type: "LineString", coordinates: animatedCoords },
            });
            // Stage 4A: update the visible Leaflet polyline + dot.
            if (__routePolyline) __routePolyline.setLatLngs(animatedCoords.map(([lng, lat]) => [lat, lng]));
            if (__routeDot) __routeDot.setLatLng([coordinates[currentPoint][1], coordinates[currentPoint][0]]);

            map.getSource('traveling-dot').setData({
              type: 'Feature',
              geometry: { type: 'Point', coordinates: coordinates[currentPoint] },
            });
            ...
            if (__routeDot && map.hasLayer(__routeDot)) map.removeLayer(__routeDot);
            if (map.getLayer('traveling-dot-layer')) {
              map.setLayoutProperty('traveling-dot-layer', 'visibility', 'none');
            }

            // Final complete path: shim store + Leaflet polyline.
            map.getSource(sourceId).setData({
              type: "Feature",
              geometry: { type: "LineString", coordinates: coordinates },
            });
            if (__routePolyline) __routePolyline.setLatLngs(coordinates.map(([lng, lat]) => [lat, lng]));
    ...
    async function drawRouteOnElevationMap(coordinates) {
      // If the report dashboard is collapsed (off-screen at bottom: -1000px),
      // the inset map's panes are unmeasured and Leaflet's fitBounds NaNs.
      // Just stash the route data — the dashboard's open handler will
      // invalidateSize + fitBounds when the user opens it.
      const bottom = exportDashboardContainer ? exportDashboardContainer.style.bottom : "";
      const dashOpen = bottom === "0px" || bottom === "0";
      if (!dashOpen) {
        // Still feed the polyline data through the elevationMap shim so the
        // inset shows the correct route as soon as it's opened.
        try {
          if (elevationMap.getSource && elevationMap.getSource("ffi-path")) {
            elevationMap.getSource("ffi-path").setData({
              type: "Feature",
              geometry: { type: "LineString", coordinates },
            });
          } else {
            elevationMap.addSource && elevationMap.addSource("ffi-path", {
              type: "geojson",
              data: { type: "Feature", geometry: { type: "LineString", coordinates } },
            });
          }
        } catch (_) { /* ignore */ }
        console.log("[elevation inset] dashboard closed; route stored, will fit on open");
        return;
      }
      ...
      return new Promise((resolve) => {
        function addRouteToElevationMap() {
          ...
          try {
            ...
            // Skip fitBounds if the inset map container is hidden / zero-sized
            // (the export dashboard is collapsed by default, so the elevation
            // inset has 0×0 pixels until the user opens the report). fitBounds
            // on a 0-size map produces NaN and throws.
            const elSize = elevationMap.getSize ? elevationMap.getSize() : { x: 0, y: 0 };
            if (elSize.x > 0 && elSize.y > 0) {
              ...
              console.log("Elevation map hidden (0 size); skipping fitBounds. Will refit when dashboard opens.");
              resolve();
            }
    ...
    async function drawRailLines(origin, destination) {
      // Calculate paths for each index
      const ffiPath = await calculateCurvedPath(origin, destination);
      console.log("[route] ffiPath length:", ffiPath?.length, "first/last:", ffiPath?.[0], ffiPath?.[ffiPath.length-1]);
      if (!ffiPath || ffiPath.length === 0) {
        throw new Error("calculateCurvedPath returned empty/null");
      }
      ...
      for (const { id, color, coordinates } of paths) {
        // Stage 4A: init/reset the visible Leaflet polyline.
        if (!__routePolyline) {
          __routePolyline = L.polyline([], {
            color: color, weight: 5, opacity: 1, lineJoin: 'round', lineCap: 'round',
            pane: "routePane",  // sits above markers/overlays
          }).addTo(map);
          console.log("[route] created __routePolyline, on map:", map.hasLayer(__routePolyline));
        } else {
          __routePolyline.setStyle({ color: color });
          __routePolyline.setLatLngs([]);
          if (!map.hasLayer(__routePolyline)) __routePolyline.addTo(map);
          console.log("[route] reused __routePolyline, on map:", map.hasLayer(__routePolyline));
        }

        // Setup or reset the shim source (for code that reads ffi-path data).
        if (!map.getSource(id)) {
          map.addSource(id, {
            type: "geojson",
            data: { type: "Feature", geometry: { type: "LineString", coordinates: [] } },
        ...
        // Stage 4A: hover popup wired to the Leaflet polyline.
        __routePolyline.off('mouseover').off('mouseout');
        __routePolyline.on('mouseover', (e) => {
          const coords = __routePolyline.getLatLngs().map(ll => [ll.lng, ll.lat]);
          const distance = turf.length(turf.lineString(coords), { units: "kilometers" }).toFixed(2);
        ...
        __routePolyline.on('mouseout', () => map.closePopup());
      }
    ...
    // Draw the initial route — wrap in try/catch so any failure surfaces
    // clearly in the console instead of dying silently inside an awaited
    // promise rejection.
    console.log("[route] drawRailLines start", { origin: originCoords, destination: destinationCoords });
    try {
      await drawRailLines(originCoords, destinationCoords);
      console.log("[route] drawRailLines done");
    } catch (err) {
      console.error("[route] drawRailLines THREW:", err);
      alert("Route calculation failed — see console (DevTools) for the error.");
    }
...
exportButton.addEventListener("click", () => {
  if (isExportDashboardOpen) {
    ...
    updateExportDashboardContent();
    // Now that the dashboard is visible, kick off the once-broken bits that
    // were waiting for visible pixels:
    //   - elevation inset (Leaflet) needs invalidateSize + fitBounds
    //   - elevation chart (Chart.js) needs resize so it paints into the now
    //     non-zero canvas instead of the 0×0 surface it was created in
    //   - route snapshot needs html2canvas to rasterise the live map div
    setTimeout(() => {
      try {
        elevationMap.invalidateSize(true);
        setTimeout(() => {
          const sz = elevationMap.getSize();
          console.log("[elevation inset] open. polyline exists:", !!__elevRoutePolyline,
            "size:", sz, "bounds:", __elevRoutePolyline?.getBounds());
          if (__elevRoutePolyline && sz.x > 0 && sz.y > 0) {
            const b = __elevRoutePolyline.getBounds();
            if (b && b.isValid()) {
              elevationMap.fitBounds(b, { padding: [30, 30], maxZoom: 12 });
              console.log("[elevation inset] fitted to route bounds");
            } else {
              console.warn("[elevation inset] polyline bounds invalid", b);
            }
          } else {
            console.warn("[elevation inset] no polyline or zero-size container");
          }
        }, 120);
      } catch (e) { console.warn("elevation map refit:", e.message); }
      // Regenerate the elevation chart fresh — the original was almost
      // certainly created in a 0×0 canvas (dashboard was collapsed) and
      // Chart.js's responsive ResizeObserver doesn't always detect the
      // bottom:-1000→0 change. Easier to just rebuild.
      try {
        if (typeof generateElevationProfile === "function") {
          generateElevationProfile().catch((e) => console.warn("elevation profile regen:", e.message));
        }
      } catch (e) { console.warn("elevation profile regen:", e.message); }
      try {
        if (typeof __regenerateSnapshot === "function") __regenerateSnapshot();
      } catch (e) { console.warn("snapshot regen:", e.message); }
    }, 400);
  }
...
// Original: map.on("sourcedata", … map.once("idle", … map.getCanvas().toDataURL))
// fails on Leaflet (no unified canvas; basemap is HTML img tiles). Rewired:
// generated on demand each time the dashboard opens via html2canvas.
const snapshotContainer = document.createElement("div");
snapshotContainer.style.flex = "0.55";

async function __regenerateSnapshot() {
  if (!map.getSource("ffi-path")) {
    snapshotContainer.innerHTML = `<div style="padding: 20px; color: #999;">No route to snapshot. Calculate a route first.</div>`;
    return;
  }
  let snapshotDataURL = "";
  try {
  ...
  __renderSnapshotPanel(snapshotDataURL);
}

function __renderSnapshotPanel(snapshotDataURL) {
...
const generateElevationProfile = async () => {
  ...
  try {
    // Stage 4F: batch Open-Elevation lookup (one request, not N).
    // Show an in-place loading bar while waiting on the network — Open-Elevation
    // can take 5-15 seconds for ~100 points. Without this the chart container
    // is just blank during the fetch.
    elevationChartContainer.innerHTML = `
      <div id="elev-loading" style="height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:10px;font-family:Inter,sans-serif;font-size:13px;color:#666;">
        <div>Fetching elevations from Open-Elevation…</div>
        <div style="width:60%;max-width:400px;height:6px;background:#e0e0e0;border-radius:3px;overflow:hidden;">
          <div id="elev-loading-bar" style="height:100%;background:#f67a0a;width:0%;transition:width 0.4s ease;"></div>
        </div>
        <div id="elev-loading-pct" style="font-size:11px;color:#999;">0%</div>
      </div>`;
    const __elevBar = document.getElementById("elev-loading-bar");
    const __elevPct = document.getElementById("elev-loading-pct");
    let __elevP = 0;
    const __elevInt = setInterval(() => {
      __elevP = Math.min(90, __elevP + 4);
      if (__elevBar) __elevBar.style.width = __elevP + "%";
      if (__elevPct) __elevPct.textContent = __elevP + "%";
    }, 250);

    // Use the multi-source elevation chain; falls back through Open-Meteo and
    // Open-Topo-Data if Open-Elevation rate-limits or fails.
    const elevations = await __fetchElevations(ffiPath);
    clearInterval(__elevInt);
    if (__elevBar) __elevBar.style.width = "100%";
    if (__elevPct) __elevPct.textContent = "100%";
    // Tear down the loading UI and put back a fresh canvas for Chart.js.
    elevationChartContainer.innerHTML = "";
...
// Stage 4H: elevation inset is now a Leaflet map with OpenTopoMap tiles
// (free, hillshaded, no key). 3D terrain is gone — Leaflet is 2D.
const elevationMap = L.map(elevationMapContainer, {
  zoomControl: false,
  attributionControl: true,
  dragging: false,
...
// Polyline state for the route shown on the elevation inset.
let __elevRoutePolyline = null;
function __updateElevRoutePolyline(coords) {
  if (!coords || coords.length === 0) return;
  const latlngs = coords.map(([lng, lat]) => [lat, lng]);
  if (!__elevRoutePolyline) {
    __elevRoutePolyline = L.polyline(latlngs, { color: "#ffa500", weight: 4, opacity: 0.95 }).addTo(elevationMap);
  } else {
    __elevRoutePolyline.setLatLngs(latlngs);
    if (!elevationMap.hasLayer(__elevRoutePolyline)) __elevRoutePolyline.addTo(elevationMap);
  }
}

// Mapbox compat shim for elevationMap (mirrors the main-map shim, just
// enough for the existing addSource/addLayer/fitBounds/flyTo/loaded calls).
const __ELEV_SOURCES = new Map();
elevationMap.addSource = (id, src) => {
  __ELEV_SOURCES.set(id, src);
  if (id === "ffi-path") __updateElevRoutePolyline(src?.data?.geometry?.coordinates || []);
};
elevationMap.removeSource = (id) => {
  __ELEV_SOURCES.delete(id);
  if (id === "ffi-path" && __elevRoutePolyline) {
    __elevRoutePolyline.remove();
    __elevRoutePolyline = null;
  }
};
elevationMap.getSource = (id) => {
  const s = __ELEV_SOURCES.get(id);
  if (!s) return null;
  return {
    _data: s.data,
    setData: (d) => { s.data = d; if (id === "ffi-path") __updateElevRoutePolyline(d?.geometry?.coordinates || []); },
  };
};
const __elevLeafletAddLayer = elevationMap.addLayer.bind(elevationMap);
const __elevLeafletRemoveLayer = elevationMap.removeLayer.bind(elevationMap);
elevationMap.addLayer = (cfgOrLayer) => {
  if (cfgOrLayer instanceof L.Layer) return __elevLeafletAddLayer(cfgOrLayer);
};
elevationMap.removeLayer = (idOrLayer) => {
  if (idOrLayer instanceof L.Layer) return __elevLeafletRemoveLayer(idOrLayer);
};
elevationMap.getLayer = () => undefined;
elevationMap.setLayoutProperty = () => {};
elevationMap.setPaintProperty = () => {};
elevationMap.setTerrain = () => {};
elevationMap.loaded = () => true;
elevationMap.resize = () => elevationMap.invalidateSize();
elevationMap.getCanvas = () => elevationMap.getContainer();

const __elevLeafletFlyTo = elevationMap.flyTo.bind(elevationMap);
const __elevLeafletFitBounds = elevationMap.fitBounds.bind(elevationMap);
elevationMap.flyTo = (opts) => {
  if (!opts) return elevationMap;
  const c = opts.center;
  if (Array.isArray(c) && c.length >= 2) {
    __elevLeafletFlyTo([c[1], c[0]], opts.zoom ?? elevationMap.getZoom(), { duration: 1 });
  }
  return elevationMap;
};
elevationMap.fitBounds = (b, opts) => {
  let bounds = b;
  // Translate Mapbox-style bbox [w,s,e,n] to Leaflet [[s,w],[n,e]]; pass any
  // other shape (L.LatLngBounds, [[lat,lng],[lat,lng]]) straight through.
  if (Array.isArray(b) && b.length === 4 && typeof b[0] === "number") {
    bounds = [[b[1], b[0]], [b[3], b[2]]];
  }
  // Forward the WHOLE options object — earlier we were squashing maxZoom +
  // animate and double-wrapping padding, which silently broke fitBounds when
  // called with already-Leaflet-shaped padding like `[30, 30]`.
  const fitOpts = { ...(opts || {}) };
  if (typeof fitOpts.padding === "number") {
    fitOpts.padding = [fitOpts.padding, fitOpts.padding];
  }
  __elevLeafletFitBounds(bounds, fitOpts);
  return elevationMap;
};

// Forward (fn, context) pair — same context-dropping bug fix as the main map.
const __elevLeafletOn = elevationMap.on.bind(elevationMap);
const __elevLeafletOnce = elevationMap.once.bind(elevationMap);
elevationMap.on = (ev, a, b, c) => {
  if (ev === "load" || ev === "style.load" || ev === "idle") {
    if (typeof a === "function") elevationMap.whenReady(a, b);
    else if (typeof b === "function") elevationMap.whenReady(b, c);
    return elevationMap;
  }
  if (ev === "sourcedata" || ev === "data") return elevationMap; // no-op; see main map shim
  if (typeof a === "string") return elevationMap; // layer-scoped, ignored
  __elevLeafletOn(ev, a, b);
  return elevationMap;
};
elevationMap.once = (ev, fn, ctx) => {
  if (ev === "load" || ev === "style.load") {
    elevationMap.whenReady(fn, ctx);
    return elevationMap;
  }
  __elevLeafletOnce(ev, fn, ctx);
  return elevationMap;
};
...
shareButton.addEventListener('click', async () => {
  ...
  try {
    ...
    // (Original `map.getCanvas().toDataURL()` was Mapbox-only — Leaflet's
    // basemap is HTML img tiles + canvas overlay, no unified canvas. The
    // snapshot panel already renders an html2canvas screenshot of the map
    // into the dashboard, which the clone picks up naturally; no extra
    // capture needed here.)
...
if (elevationMap) {
  try {
    ...
    // No fallback — `elevationMap.getCanvas().toDataURL()` is Mapbox-only;
    // skip the inset image rather than throw.
    elevationMapImage = null;
  }
} else {
